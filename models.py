...
class ItemTextures:
    ...
    character: List[str] = field(default_factory=list)  # 手持/穿戴状态贴图
    character_left: List[str] = field(default_factory=list)  # 左手贴图
    inventory: List[str] = field(default_factory=list)  # 物品栏贴图列表
    loot: List[str] = field(default_factory=list)  # 战利品贴图
    ...
    def is_animated(self, field: str) -> bool:
        """指定字段是否为动画"""
