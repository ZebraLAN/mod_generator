...
WEAPONS_SLOT = ["sword", "axe", "mace", "dagger", "twohandedsword", "spear", 
                "twohandedaxe", "twohandedmace", "bow", "crossbow", "sling", 
                "twohandedstaff", "chain", "lute"]
WEAPONS_RARITY = ["Common", "Unique"]
WEAPONS_MATERIAL = ["wood", "metal", "leather"]
WEAPONS_TAGS = ["unique", "magic", "special", "special exc", "WIP", "aldor", 
                "aldor common", "aldor uncommon", "aldor rare", "aldor magic", 
                "fjall", "elven", "skadia", "nistra"]
...
ATTRIBUTE_DESCRIPTIONS = {
    ...
    'Bleeding_Chance': ('出血几率', '你的攻击命中目标的身体部位之后，有这么大的几率造成~r~出血~/~。'),
    'Knockback_Chance': ('击退几率', '你的攻击有这么大的几率令目标~w~后退~/~一个方格。'),
    ...
    'Spells_Energy_Cost': ('咒法精力消耗', '这个属性影响咒法实际所耗精力与其正常所耗精力的比值。'),
    'Skills_Energy_Cost': ('技能精力消耗', '这个属性影响技能实际所耗精力与其正常所耗精力的比值。'),
    'Abilities_Energy_Cost': ('能力精力消耗', '这是技能和咒法实际所耗精力与其正常所耗精力的比值。'),
    'Cooldown_Reduction': ('冷却时间', '这是所有能力实际冷却时间与其正常冷却时间的比值。'),
...
SUPPORTED_LANGUAGES = [
    "English", "Русский", "Deutsch", "Español (LATAM)", 
...
class Weapon:
    ...
    def validate(self, project=None) -> List[str]:
        ...
        if not self.name:
            ...
            errors.append("武器系统ID格式错误: 必须以字母开头，只能包含字母、数字和空格")
        ...
        if project:
            ...
            for w in project.weapons:
                if w.id == current_id:
                    count += 1
            if count > 1:
                errors.append(f"武器系统ID '{self.name}' (ID: {current_id}) 重复，请确保唯一")
        ...
        if not self.textures.inventory or not any(tex for tex in self.textures.inventory):
...
class ModProject:
    ...
    def save(self, file_path: str = None):
        ...
        for weapon in self.weapons:
            ...
            # 处理贴图路径：转换为相对路径
            rel_char_path = self._get_relative_path(weapon.textures.character, project_dir)
            rel_char_left_path = self._get_relative_path(weapon.textures.character_left, project_dir)
            rel_loot_path = self._get_relative_path(weapon.textures.loot, project_dir)
            rel_inv_paths = [self._get_relative_path(p, project_dir) for p in weapon.textures.inventory]
            ...
            weapon_data = {
                ...
                "textures": {
                    ...
                    "character_frames": [self._get_relative_path(p, project_dir) for p in weapon.textures.character_frames],
                    "character_fps": weapon.textures.character_fps,
                    "character_left_frames": [self._get_relative_path(p, project_dir) for p in weapon.textures.character_left_frames],
                    "character_left_fps": weapon.textures.character_left_fps,
                    "loot_frames": [self._get_relative_path(p, project_dir) for p in weapon.textures.loot_frames],
    ...
    def _get_relative_path(self, path: str, project_dir: str) -> str:
        ...
        try:
            ...
            if os.path.commonpath([project_dir, os.path.abspath(path)]) == os.path.abspath(project_dir):
    ...
    def load(self, file_path: str):
        ...
        self.description = data.get("description", "使用 Stoneshard Weapon Mod Editor 生成")
        ...
        for weapon_data in data.get("weapons", []):
            ...
            # 解析帧动画路径
            char_frames = [resolve_path(p) for p in tex_data.get("character_frames", [])]
            char_left_frames = [resolve_path(p) for p in tex_data.get("character_left_frames", [])]
...
class ModGeneratorGUI:
    def __init__(self):
        ...
        self.window = glfw.create_window(1200, 800, "Stoneshard 武器模组编辑器", None, None)
        if not self.window:
            glfw.terminate()
            print("无法创建 GLFW 窗口")
            sys.exit(1)
        ...
        self.attribute_groups = {
            "伤害类型": ['Slashing_Damage', 'Piercing_Damage', 'Blunt_Damage', 'Rending_Damage',
                      'Fire_Damage', 'Shock_Damage', 'Poison_Damage', 'Caustic_Damage', 'Frost_Damage',
                      'Arcane_Damage', 'Unholy_Damage', 'Sacred_Damage', 'Psionic_Damage'],
            "战斗属性": ['Hit_Chance', 'CRT', 'CRTD', 'CTA', 'PRR', 'Block_Power', 'Block_Recovery', 'FMB'],
            "穿透破坏": ['Armor_Piercing', 'Armor_Damage', 'Bodypart_Damage'],
            "状态效果": ['Bleeding_Chance', 'Knockback_Chance', 'Daze_Chance', 'Stun_Chance', 
                      'Immob_Chance', 'Stagger_Chance'],
            "吸血回复": ['Lifesteal', 'Manasteal'],
            "生存属性": ['max_hp', 'Health_Restoration', 'Healing_Received', 'Crit_Avoid', 'Damage_Received'],
            "精力相关": ['MP', 'MP_Restoration'],
            "能量消耗": ['Abilities_Energy_Cost', 'Skills_Energy_Cost', 'Spells_Energy_Cost', 'Cooldown_Reduction'],
            "魔法属性": ['Magic_Power', 'Miscast_Chance', 'Miracle_Chance', 'Miracle_Power'],
            "元素法力": ['Pyromantic_Power', 'Geomantic_Power', 'Venomantic_Power', 'Cryomantic_Power',
    ...
    def reload_fonts(self):
        ...
        if not target_fallback_path:
            default_bundled = os.path.join("fonts", "chinese", "HanyiSentyYongleEncyclopedia-2020.ttf")
        ...
        if target_fallback_path:
            try:
                ...
                # 尝试微软雅黑作为最后的备用
                if "msyh" not in target_fallback_path and os.path.exists("C:/Windows/Fonts/msyh.ttc"):
                    try:
                        print("尝试合并微软雅黑...")
                        self.io.fonts.add_font_from_file_ttf(
    ...
    def get_bundled_fonts(self, subdir):
        ...
        if not os.path.exists(path):
            return []
        files = [f for f in os.listdir(path) if f.lower().endswith(('.ttf', '.ttc', '.otf'))]
        return files
    ...
    def draw_common_popups(self):
        ...
        if imgui.begin_popup_modal("生成成功")[0]:
            ...
            # 计算实际输出路径
            base_dir = os.path.dirname(self.project.file_path) if self.project.file_path else "."
            mod_dir = os.path.abspath(os.path.join(base_dir, self.project.code_name.strip() or "ModProject"))
            ...
            if imgui.begin_menu("设置", True):
                ...
                if imgui.begin_menu("字体大小"):
                    for size in [14, 16, 18, 20, 24, 28, 32]:
                         if imgui.menu_item(f"{size} px", selected=(self.font_size == size))[0]:
                             self.font_size = size
                             self.save_config()
    ...
    def draw_main_interface(self):
        ...
        menu_bar_height = imgui.get_frame_height() # 如果就在菜单栏绘制之后调用，这通常能拿到正确高度
        # 或者更稳妥的方式：
        ...
        imgui.begin("Main Interface", flags=imgui.WINDOW_NO_RESIZE | imgui.WINDOW_NO_MOVE | imgui.WINDOW_NO_COLLAPSE | imgui.WINDOW_NO_TITLE_BAR)
        ...
        if not self.project.file_path:
            ...
            imgui.set_cursor_pos(((window_width - text_width) / 2, window_height / 2 - 20))
            imgui.text(text)
            
            # 添加按钮快捷方式
            button_width = 120
            imgui.set_cursor_pos((window_width / 2 - button_width - 10, window_height / 2 + 20))
    ...
    def draw_project_info(self):
        changed, self.project.name = imgui.input_text("模组名称", self.project.name, 256)
        if imgui.is_item_hovered():
            imgui.set_tooltip("用于展示的名称，可包含中文等字符")
        
        changed, self.project.code_name = imgui.input_text("模组代号", self.project.code_name, 256)
        if imgui.is_item_hovered():
            imgui.set_tooltip("仅用于内部生成代码，必须是以字母开头的字母/数字组合")
        
        changed, self.project.author = imgui.input_text("作者", self.project.author, 256)
        changed, self.project.description = imgui.input_text("描述", self.project.description, 512)
        changed, self.project.version = imgui.input_text("版本", self.project.version, 32)
        changed, self.project.target_version = imgui.input_text("目标版本", self.project.target_version, 32)
        
        imgui.text(f"项目路径: {os.path.dirname(self.project.file_path) if self.project.file_path else '未保存'}")
    ...
    def draw_weapon_editor(self):
        ...
        if imgui.tree_node("基本属性", flags=imgui.TREE_NODE_FRAMED):
            ...
            if imgui.is_item_hovered():
                imgui.set_tooltip("用来让游戏识别该物品的内部名称，不向玩家展示。\n请确保ID尽可能独特，以免与其他Mod冲突！")
            imgui.same_line()
            imgui.text(f"(ID: {weapon.id})")
            
            # 枚举选择
            current_tier = WEAPONS_TIER.index(weapon.tier) if weapon.tier in WEAPONS_TIER else 0
            ...
            current_slot = slot_options.index(weapon.slot) if weapon.slot in slot_options else 0
            ...
            current_mat = WEAPONS_MATERIAL.index(weapon.mat) if weapon.mat in WEAPONS_MATERIAL else 0
            ...
            # 稀有度 - 禁用直接操作，只显示
            rarity_label = RARITY_LABELS.get(weapon.rarity, weapon.rarity)
            imgui.input_text("稀有度", rarity_label, 256, flags=imgui.INPUT_TEXT_READ_ONLY)
            if imgui.is_item_hovered():
            ...
            changed, weapon.max_duration = imgui.input_int("最大耐久", weapon.max_duration)
            
            # Rng 锁定逻辑: 仅弓弩可调，其他锁定为 1
            if weapon.slot in ["bow", "crossbow"]:
                changed, weapon.rng = imgui.input_int("距离", weapon.rng)
                if changed:
                    if weapon.rng < 0: weapon.rng = 0
                    if weapon.rng > 255: weapon.rng = 255
                if imgui.is_item_hovered():
        ...
        if errors:
            ...
            for error in errors:
                if error.startswith("WARNING:"):
                    imgui.text_colored(f"  • {error}", 1.0, 0.5, 0.0, 1.0) # Orange for warning
                else:
                    imgui.text_colored(f"  • {error}", 1.0, 0.0, 0.0, 1.0) # Red for error
        ...
        for group_name, attributes in self.attribute_groups.items():
            if imgui.tree_node(group_name):
                for attr in attributes:
                    ...
                    changed, new_val = imgui.input_int(f"##{attr}", val, step=1, step_fast=10)
    ...
    def draw_localization_editor(self, weapon):
        ...
        if imgui.begin_popup("add_language_popup"):
            for lang in SUPPORTED_LANGUAGES:
                if lang not in weapon.localization.other_languages and lang != "Chinese":
                     if imgui.selectable(lang)[0]:
                         weapon.localization.other_languages[lang] = {"name": "", "description": ""}
            imgui.end_popup()
        ...
        changed, weapon.localization.chinese_name = imgui.input_text("##chn_name", weapon.localization.chinese_name, 256)
        ...
        changed, weapon.localization.chinese_description = imgui.input_text_multiline(
            "##chn_desc", 
            weapon.localization.chinese_description, 
            1024, 
            height=60
        )
        imgui.pop_item_width()
        ...
        for lang, data in weapon.localization.other_languages.items():
            ...
            imgui.text("描述")
            imgui.push_item_width(-1)
            changed, val = imgui.input_text_multiline(f"##{lang}_desc", data["description"], 1024, height=60)
            if changed:
    ...
    def draw_textures_editor(self, weapon):
        ...
        self.draw_texture_selector("手持状态贴图*", weapon.textures.character, "character", weapon)
        ...
        if imgui.is_item_hovered():
            imgui.set_tooltip("排在后面的贴图代表更低耐久状态下的武器\n注意：游戏内一格为 27 像素")
        
        for idx, texture_path in enumerate(weapon.textures.inventory):
            self.draw_texture_selector(f"常规贴图 {idx + 1}", texture_path, ("inventory", idx))
    ...
    def draw_texture_selector(self, label: str, current_path: str, field_identifier, weapon=None):
        imgui.text(label)
        imgui.same_line()
        ...
        if is_anim_field and frames:
                ...
                if paths:
                    ...
                    for path in paths:
                        ...
                        if self.project.file_path:
                             ...
                             final_path = os.path.join(os.path.dirname(self.project.file_path), rel_path)
                        else:
                             final_path = path
            ...
            # 预览控制 (暂停/指定帧)
            state = self.preview_states.get(field_identifier, {"paused": False, "current_frame": 0})
            imgui.same_line()
            ...
            if state["paused"] and len(frames) > 0:
                ...
                changed, current_frame_1based = imgui.slider_int(f"##frame_slider_{label}", current_frame_1based, 1, max_frame + 1, format="%d")
                if changed:
            ...
            if imgui.button(button_id):
                ...
                if paths:
                    ...
                    if self.project.file_path:
                         ...
                         final_path_0 = os.path.join(os.path.dirname(self.project.file_path), rel_path)
                    else:
    ...
    def draw_preview_with_reference(self, preview, field_identifier, override_size=None):
    ...
    def apply_texture_selection(self, file_path: str, field_identifier=None) -> bool:
        ...
        if self.current_weapon_index < 0 or self.current_weapon_index >= len(self.project.weapons):
            return False
        weapon = self.project.weapons[self.current_weapon_index]
        ...
        if isinstance(field, tuple):
            ...
            if field_type == "inventory" and idx is not None and 0 <= idx < len(weapon.textures.inventory):
    ...
    def draw_import_dialog(self):
        ...
        if imgui.begin_popup_modal("导入项目")[0]:
            ...
            changed, self.import_file_path = imgui.input_text("文件路径", self.import_file_path, 512)
            ...
            if imgui.button("确定"):
                if os.path.exists(self.import_file_path):
                    success, message, conflicts = self.project.import_project(self.import_file_path)
                    if success:
                        if conflicts:
    ...
    def draw_texture_dialog(self):
        ...
        if imgui.begin_popup_modal("选择贴图文件")[0]:
            ...
            changed, self.temp_texture_path = imgui.input_text("##path", self.temp_texture_path, 512)
            imgui.pop_item_width()
    ...
    def save_file_dialog(self, default_ext=".json", file_types=None):
        ...
        try:
            ...
            file_path = filedialog.asksaveasfilename(defaultextension=default_ext, filetypes=ftypes)
            root.destroy()
            return file_path
        except Exception as e:
            print(f"保存文件对话框错误: {e}")
    ...
    def generate_mod(self):
        ...
        if project_errors:
            print("错误: 项目验证失败")
            self.error_message = "项目验证失败:\n" + "\n".join(f"  • {err}" for err in project_errors)
            self.show_error_popup = True
            return
            ...
            for weapon in self.project.weapons:
                ...
                if weapon.textures.loot_frames:
                    for idx, frame_path in enumerate(weapon.textures.loot_frames):
                        self.copy_texture(frame_path, sprites_dir / f"s_loot_{weapon_id}_{idx}.png")
                else:
                    self.copy_texture(weapon.textures.loot, sprites_dir / f"s_loot_{weapon_id}.png")
...
"""
    ...
    def generate_weapon_method(self, weapon: Weapon) -> str:
        ...
        eng_data = weapon.localization.other_languages.get("English", {"name": "", "description": ""})
        ...
        if weapon.textures.offset_x != 0 or weapon.textures.offset_y != 0 or weapon.textures.offset_x_left != 0 or weapon.textures.offset_y_left != 0:
            ...
            # 处理左手手持
            if weapon.textures.character_left and (weapon.textures.offset_x_left != 0 or weapon.textures.offset_y_left != 0):
